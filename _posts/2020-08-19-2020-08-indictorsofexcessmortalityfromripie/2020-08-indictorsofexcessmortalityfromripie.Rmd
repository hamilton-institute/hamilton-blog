---
title: "Re-emerging Indictors of Excess Mortality in Ireland"
description: |
...
ref_level <- town_in_rk_count_geo_merged %>% filter(Year < 2020 & Year >=2015 ) %>% 
  ungroup() %>%
  group_by(Group,Date) %>% 
...
prop_plot_data <- as_tibble(town_in_rk_count_geo_merged) %>% 
  filter(Year == 2020) %>%
